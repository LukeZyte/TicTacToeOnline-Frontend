...
const Auth = () => {
  ...
  const onSignInSubmit = (data: FormData) => {
    ...
    axiosInstance.post(ApiEndpoints.AuthLogin, {
      login: data.login,
      password: data.password,
    }).then(response => {
      console.log('LOG IN:', response.data);
      sessionStorage.setItem(SessionStorageEnum.Token, response.data.token);
      sessionStorage.setItem(SessionStorageEnum.ExpiryDate, response.data.expiryDate);
      ...
      <Container maxWidth="sm" style={{alignSelf: "center"}}>
        <Card variant='outlined' style={{padding: 32}}>
          ...
          <form onSubmit={handleSubmit(onSignInSubmit)}>
            <Stack gap={1} marginTop={4}>
              <FormControl fullWidth error={!!errors.login}>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <OutlinedInput id="login" label="Login" {...field} />
                  )}
                />
            ...
            <Stack gap={1} marginTop={4}>
              <Button variant="contained" size="large" type="submit" loading={loginLoading}>Sign in</Button>
              <Button variant="outlined" onClick={onSignUpClick}>Create a new account</Button>
            </Stack>
          </form>
        </Card>
      </Container>
